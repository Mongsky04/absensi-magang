...
const AbsensiAPI = {
  ...
  laporAbsen: async (formData) => {
    try {
      const res = await api.post("/absensi/lapor", formData);
      return res.data;
    } catch (err) {
      throw err.response?.data || err;
    }
  ...
  getLaporan: async () => {
    try {
      const res = await api.get("/absensi/laporan");
      return res.data.data || [];
    } catch (err) {
      throw err.response?.data || err;
    }
  ...
  // Get all laporan (for admin)
  getAllLaporan: async () => {
    try {
      const res = await api.get("/absensi/laporan/all");
      return res.data.data || [];
    } catch (err) {
      throw err.response?.data || err;
    }
  ...
  // Approve/Reject laporan (for admin)
  respondLaporan: async (laporanId, payload) => {
    try {
      const res = await api.put(`/absensi/laporan/${laporanId}`, payload);
      return res.data.data;
    } catch (err) {
      throw err.response?.data || err;
    }
