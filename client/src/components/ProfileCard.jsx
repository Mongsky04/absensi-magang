  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="p-3 sm:p-5">
        ...
        {/* Periode Magang */}
        {(user?.tanggalMulai || user?.tanggalSelesai) && (
          <div className={`${user?.mentor ? 'mt-2' : 'mt-3 pt-3 border-t border-slate-100'}`}>
            <p className="text-xs text-slate-500">
              <span className="font-medium">Periode Magang:</span>{" "}
              {user.tanggalMulai && new Date(user.tanggalMulai).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}
              {user.tanggalMulai && user.tanggalSelesai && " - "}
              {user.tanggalSelesai && new Date(user.tanggalSelesai).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}
            </p>
          </div>
        )}
      </div>
