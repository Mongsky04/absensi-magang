...
export default function PresensiBox({ onSubmit, onPulang, todayRecord }) {
  ...
  const handleCheckIn = async () => {
    ...
    // Validasi keterangan mandatory jika telat (>= 08:00)
    if (currentHour >= 8 && !keterangan.trim()) {
      alert("Keterangan wajib diisi karena Anda check-in setelah jam 08:00");
      return;
    }
  ...
  const handleCheckOut = async () => {
    ...
    if (currentHour < 8) {
      alert(`Check-out hanya dapat dilakukan mulai jam 08:00. Sekarang jam ${formatTime(now)}.`);
      return;
    }

    // Validasi keterangan mandatory jika check-out sebelum jam 17:00
    if (currentHour < 17 && !keterangan.trim()) {
      alert("Keterangan wajib diisi karena Anda check-out sebelum jam 17:00");
      return;
    }
      ...
      <div className="p-4">
        ...
        {!hasCheckedIn && (
          <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-xs text-blue-800">
              ...
              <span className="text-[10px] ml-6 block">
                • Sebelum 08:00: Status <strong>Hadir</strong>, keterangan opsional<br/>
                • Setelah 08:00: Status <strong>Telat</strong>, keterangan <strong className="text-red-600">wajib diisi</strong>
              </span>
            </p>
          </div>
        )}
          ...
          <div className="mb-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-xs text-amber-800">
              <span className="flex items-center gap-2 mb-1">
                ...
                Check-out dapat dilakukan mulai jam <strong>08:00</strong>
              </span>
              <span className="text-[10px] ml-6 block">
                • Sebelum 17:00: Keterangan <strong className="text-red-600">wajib diisi</strong><br/>
                • Setelah 17:00: Keterangan opsional
              </span>
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <button
            ...
            disabled={loading || !hasCheckedIn || hasCheckedOut || (hasCheckedIn && !hasCheckedOut && new Date().getHours() < 8)}
          ...
          >
            ...
            <span className={`text-xs mt-0.5 ${hasCheckedOut ? "text-green-500" : "text-slate-400"}`}>
              {hasCheckedOut
                ...
                : hasCheckedIn && new Date().getHours() < 8
                ? "Tersedia 08:00"
                : "00:00:00"}
            </span>
          </button>
        ...
        <div className="mt-4">
          {(() => {
            ...
            const isCheckInTime = !hasCheckedIn;
            const isCheckOutTime = hasCheckedIn && !hasCheckedOut;

            let placeholder = "Tambahkan keterangan (opsional)";
            let isMandatory = false;

            if (isCheckInTime && currentHour >= 8) {
              placeholder = "Keterangan wajib diisi (telat check-in)";
              isMandatory = true;
            } else if (isCheckOutTime && currentHour < 17) {
              placeholder = "Keterangan wajib diisi (check-out sebelum jam 17:00)";
              isMandatory = true;
            }
            ...
            return (
              <>
                <textarea
                  ...
                  className={`w-full border rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:border-transparent resize-none ${
                    isMandatory
