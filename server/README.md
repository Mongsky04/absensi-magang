...
```
MONGO_URI=mongodb://localhost:27017/absensi_magang
CLOUDINARY_CLOUD_NAME=xxx
CLOUDINARY_API_KEY=111111
CLOUDINARY_API_SECRET=yyyyyyyy
SESSION_SECRET=sesi-rahasia
ADMIN_SECRET=admin-create-secret
CORS_ORIGIN=http://localhost:5173
PORT=5000
```

Jika belum punya Cloudinary:

1. Daftar di https://cloudinary.com/ (free tier cukup).
2. Masuk Dashboard → Settings → Copy Cloud name, API Key, API Secret.
3. Isi ke `.env` sesuai di atas.

Upload alur (absen masuk/pulang):

1. Frontend webcam ambil dataURL → konversi ke Blob → FormData field `foto`.
2. Endpoint `/api/absensi/masuk` atau `/pulang` menerima multipart → `multer` memory → upload ke Cloudinary folder `absensi-presensi`.
3. `secure_url` disimpan di field `foto` dokumen Absensi.
4. Tabel frontend render `<img src={row.foto} />`.

Troubleshooting:

- Jika muncul `PayloadTooLargeError`, pastikan sudah pakai multipart dan bukan JSON base64 besar.
- Jika 401 setelah upload: token kadaluarsa, login ulang.
- Jika Cloudinary error `Invalid Signature`: periksa API Secret typo.
- Jika gambar tidak muncul: cek Inspector Network → response `data.foto` harus URL `https://res.cloudinary.com/...`.
