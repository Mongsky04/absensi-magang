...
export const absenMasuk = async (req, res) => {
  try {
    ...
    let status = currentHour >= 8 ? "telat" : "hadir";
    const { keterangan = "", status: statusOverride } = req.body || {};

    // Validasi: Keterangan mandatory jika telat (jam >= 08:00)
    if (status === "telat" && !keterangan.trim()) {
      return res.status(400).json({
        message: "Keterangan wajib diisi karena Anda check-in setelah jam 08:00",
        requireKeterangan: true
      });
    }
...
export const absenPulang = async (req, res) => {
  try {
    ...
    // Validasi: Check-out dibuka dari jam 08:00 sampai esok hari jam 06:00
    const currentHour = now.hour();

    if (currentHour < 8) {
      return res.status(400).json({
        message: `Check-out hanya dapat dilakukan mulai jam 08:00. Sekarang jam ${now.format('HH:mm')}.`,
        currentTime: now.format('HH:mm'),
        allowedTime: '08:00 - 06:00 (esok hari)'
      });
    }

    const { keterangan = "" } = req.body || {};

    // Validasi: Keterangan mandatory jika check-out sebelum jam 17:00
    if (currentHour < 17 && !keterangan.trim()) {
      return res.status(400).json({
        message: "Keterangan wajib diisi karena Anda check-out sebelum jam 17:00",
        requireKeterangan: true
      });
    }
